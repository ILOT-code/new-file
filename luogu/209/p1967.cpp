//设两点x,y,现在证明图中两点间最大运载量就是在最大生成树上的最大运载量。
//设在最大生成树中，x,y两点路径运载量最小的边e.
//那么在krus运行过程中，当找到e这条边时，该路径上其它边已经全部找到。
//如果错误，就说明，原图中有那么一条路径连接x,y，每一条边的运载量都大于e的。
//在krus运行过程中，上面的边都在e之前被选择到。
//这些边有三种可能：
//1:构成最大生成树上x,y路径上，除e之外的其他边。 这对x,y运载量无影响
//2:在除x,y的联通集之外的集合中构成环，或联通集合（除x,y）.对x,y运载量无影响
//3:在x或y联通集内构成环。 x,y之间运载量依旧取决与e，对x,y运载量无影响
//则得证。
#include<cstdio>
#include<cstring>
#include<algorithm>
